...
def explain_api_error(e) -> str:
    ...
    if "credit balance" in body.lower():
        return "ΤΕΛΕΙΩΣΕ Η ΠΙΣΤΩΣΗ στο console.anthropic.com — κάνε top-up στο Billing"
    if e.code == 429:
        return "Όριο ρυθμού (rate limit) — θα περάσει μόνο του στο επόμενο τρέξιμο"
    if e.code in (401, 403):
        return "Πρόβλημα με το ANTHROPIC_API_KEY — έλεγξε το Secret στο GitHub"
...
ΥΠΟΧΡΕΩΤΙΚΟ: Κάθε θέμα (στα ΚΥΡΙΟΤΕΡΑ, στο «Αξίζει άρθρο» και στο
«Τουρκικό αφήγημα») κλείνει με την πηγή του σε παρένθεση, με το πλήρες
όνομά της όπως δίνεται στο πεδίο source — π.χ. (Cyprus Mail),
(Anadolu Agency (EN)), (Nordic Monitor). Αν ένα θέμα καλύπτεται από
πολλές πηγές, ανάφερε έως δύο. Ποτέ θέμα χωρίς πηγή.
...
def build_digest() -> str | None:
    ...
    try:
        ...
        print(f"[!] Claude API: {explain_api_error(e)}")
        sys.exit(1)
